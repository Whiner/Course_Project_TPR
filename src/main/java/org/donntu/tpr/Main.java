...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = st; i < st + kol; i++) {
            ...
            table[i - st][0] = (double) i;
            table[i - st][1] = Statistics.getInstance().getAvgCarsWaitingOnBakery();
            table[i - st][2] = Statistics.getInstance().getAvgCarsWaitingOnStores();
            table[i - st][3] = Statistics.getInstance().getAvgBakeryDowntime();
            table[i - st][4] = Statistics.getInstance().getAvgStoresDowntime();
